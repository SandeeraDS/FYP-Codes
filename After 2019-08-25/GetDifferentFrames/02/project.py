...
def detect_text(img, img_dialate, frame_position):
    ...
    img_empty = 255 * np.ones([height, width], dtype=np.uint8)
    # img_empty.fill(255)
    # Find Contours
    ...
    for r in list:
        ...
        # blur=img[r[1]:r[1]+r[3], r[0]:r[0]+r[2]]
        # blur = cv2.GaussianBlur(img[r[1]:r[1] + r[3], r[0]:r[0] + r[2]], (3, 3), 0)
        # blur = cv2.blur(img[r[1]:r[1] + r[3], r[0]:r[0] + r[2]], (3, 3))
        # # blur = cv2.adaptiveThreshold(blur, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 7)
